...
import { simulatePDA } from './pdaEngine';
import { pdaPresets } from './pdaPresets';
import { pdaTheory } from './pdaTheory';
...
function PDAView() {
  ...
  const activeStates =
    trace.length > 0 && trace[currentStepIndex]
      ? trace[currentStepIndex].state != null
        ? [trace[currentStepIndex].state]
        : []
      : [];
  ...
  const currentPreset = machine
    ? pdaPresets.find((p) => p.id === machine.id) || null
    : null;

  const currentStack =
    trace.length > 0 && trace[currentStepIndex]
      ? trace[currentStepIndex].stack || []
      : [];
  ...
  function handleLoadExample() {
    const currentIdx = machine
      ? pdaPresets.findIndex((p) => p.id === machine.id)
      : -1;
    const nextIdx = (currentIdx + 1) % pdaPresets.length;
    loadPreset(pdaPresets[nextIdx]);
  }
  ...
  return (
    <>
      ...
      <div className="left-panel">
        <TheoryPanel theory={pdaTheory} preset={currentPreset} />
        <PresetSelector
          presets={pdaPresets}
